...
using Pellychan.GUI.Framework.Extensions;
    ...
    public class Scheduler
    {
        ...
        public void Add(ScheduledDelegate task)
        {
            ...
            lock (queueLock)
            {
                ...
                {
                    Console.WriteLine($"{this} has {timedTasks.Count} timed tasks pending"/*, LoggingTarget.Performance*/);
                    Console.WriteLine($"- First task: {timedTasks.First()}"/*, LoggingTarget.Performance*/);
                    Console.WriteLine($"- Last task: {timedTasks.Last()}"/*, LoggingTarget.Performance*/);
                }
            }
        }
        ...
        private void enqueue(ScheduledDelegate task)
        {
            lock (queueLock)
            {
                ...
                if (runQueue.Count % LOG_EXCESSSIVE_QUEUE_LENGTH_INTERVAL == 0)
                    Console.WriteLine($"{this} has {runQueue.Count} tasks pending"/*, LoggingTarget.Performance*/);
            }
        }
    }
}